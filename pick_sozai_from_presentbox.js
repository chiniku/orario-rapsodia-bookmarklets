...
(function() {
  ...
  const regex_config = {
    ...
    , "素材": () => "(?<!防御の)石|玉|木|綿|舌|薬草|ネグルド"
  };

  let target_items = new RegExp(regex_config["素材"]());
  ...
  async function main() {
    if (prompt === true){
      const user_input = window.prompt("素材名を入力して下さい(正規表現も可)", "例) ゴブリンの角");
      ...
      max_receive_count  = parseInt(user_input_count, 10);
  ...
  function get_present(target_items){
    return async () => {
      ...
      while (items = search_item(target_items), items.length) {
        ...
        click_receive(node);
        receive_count += 1;
        console.log("受け取り回数:", receive_count);
